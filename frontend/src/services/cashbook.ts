...
export const cashbookService = {
  ...
  async markExpensesPaid(ids: string[], paid_date?: string): Promise<{ marked: number; paid_date: string }> {
    const { data } = await api.post('/cashbook/pending-expenses/mark-paid', { ids, paid_date: paid_date || null })
    return data
  },
}
